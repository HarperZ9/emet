...
> and it closes a loop opened in [§1](./01-is-ought-seam.md),
> [§5](./05-authored-root.md), and [§6](./06-aleph.md). A reader who knows only
> `SPEC.md` can follow it; every term it leans on is in the GLOSSARY.
...
from becoming a universal solvent that explains everything and therefore
forbids nothing. So the placement of EMET has to earn its slot the same way
every other case does: by surviving the subtraction test.
...
to the load-bearing argument only when removing the membrane-reading would
remove a real constraint, and demoted to mere illustration when it would not.

The taxonomy then sorts the surviving claims by *how* the structure holds. A
...
These three labels are not a courtesy ranking. They are a fence. The literal
cases carry the weight; the isomorphic cases borrow the *form* of the literal
cases under an explicit "same structure, different kind" label; the lineage
cases are quarantined so they cannot smuggle unfalsifiable content into a
falsifiable argument. Confidence-labeled paraphrase of the originating analysis:
the corpus through-line treats this fence as the difference between a thesis
that says something false-if-wrong and a slogan that says nothing (high
confidence this is its stated discipline; see *Further reading*).

## The taxonomy restated: the three tiers

I restate the through-line's placements here so this essay is self-contained;
the through-line is *further reading*, not the warrant. The warrant is the
subtraction test applied case by case, which the reader can re-run.

### Literal: the neuron and the firewall

Two cases instantiate all three membrane-properties with no metaphor at all, and
they are the load-bearing anchors that show the form is real before any
analogy is drawn.
...
- **Authentication is the 1|0 of the signal.** Is this the genuine, bit-intact,
  correctly-anchored stream it claims to be? Necessary; not the deciding layer.
...
case. Confidence note: the existence of authentication-versus-authorization as
real, distinct security layers (and confused-deputy / replay as the predicted
failure modes of collapsing them) is high confidence; the membrane *framing* of
that split is interpretive (moderate). The placement below leans only on the
...
that imported those claims into the verifier would be over-reaching.
...
for the byte seam EMET actually decides, illumination for the theological figure
that brands it. This essay inherits that discipline and adds nothing to it. The
aleph is *why the curation reaches for "membrane"*; it is not *why any EMET
boundary is shaped the way it is*. Marking it as lineage is what keeps the
figure from doing work it cannot do.

## Placing EMET: literal, authentication-side, refusing the crossing

Now the placement, run through the subtraction test so it earns its slot.
...
This survives the subtraction test. Drop the membrane reading and you lose a
real constraint: you lose the reason the lattice is *closed* (no `TRUSTED`, [§2](./02-no-aseity.md)),
the reason the hash is over *exact raw bytes* with no normalization
([§3](./03-occasionalism.md)), and the reason EMET cannot ground itself
...
sentence: **the authorization membrane is real and necessary, and EMET is not
it.** A reader who walks away thinking "EMET is the membrane that confers the
ought" has misread the placement; the next section answers that reader directly.
...
license it, which is exactly the laundering [§1](./01-is-ought-seam.md) shows
...
against, and the check is mechanical: run the lattice, read the boundary tests,
look for a verdict that asserts authority. If you find one, the taxonomy
mislocated EMET.

## Status

**Load-bearing, and marked literal.** The membrane reading of EMET is literal
(kin to the firewall, all three properties without metaphor) and it is
...
## The close, applied to this essay
...
verifier that refuses to be its own root of trust ([§5](./05-authored-root.md))
is described by an essay that refuses to be its own warrant. The placement holds
only as long as you can re-run the subtraction test and watch it forbid
something. Re-run it.
...
If a source and `SPEC.md` disagree, `SPEC.md` governs and the essay is wrong.

- The three-tier sort (literal / isomorphic / lineage), the subtraction test, and
  the firewall's authentication-vs-authorization split:
  `research/dissertation/membrane-through-line.md`, §§4–6.
- The is/ought location (the seam is *located*, never *crossed*, by the word
  "membrane"): same source, §6, and [§1](./01-is-ought-seam.md) of this curation.
- The corpus laws referenced by the sibling essays this one cross-links:
  `research/CATALOG.md` (L1, L2, L6, L8, L11).
...
- [GLOSSARY](./GLOSSARY.md): literal / isomorphic / lineage; materialized grant;
  residual structural aseity; aleph / *emet* / *met*; and every other term used
  above.
