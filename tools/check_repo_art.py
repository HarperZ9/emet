...
the gates run as a script and emit a receipt, and test_repo_art.py asserts on
that receipt under unittest, alongside the other root-level test scripts.
"""
...
def check_seed_is_recorded(_unused: list[Path]) -> list[str]:
    ...
    svg = ART_LIB.header_svg(
        {"name": "emet", "role": "x", "tagline": "y", "words": []})
    stamp = f"SEED {ART_LIB.seed_for('emet') % 100000:05d}"
